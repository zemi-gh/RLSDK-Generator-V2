...
namespace Generator
{
    ...
    bool Initialize(bool bCreateLog)
    {
        if (!GlobalsInitialized)
        {
            ...
            {
                uintptr_t gnames = 0, gobjects = 0;
                if (!OffsetScanner::Resolve(gnames, gobjects))
                {
                    Utils::Messagebox("Failed to scan for GNames/GObjects offsets.", (MB_OK | MB_ICONERROR));
                    return false;
                }
                ...
                std::ostringstream m;
                const uintptr_t entry = Retrievers::GetEntryPoint();
                m << "Runtime scan succeeded\n";
                m << "EntryPoint: " << Printer::Hex(entry, sizeof(uintptr_t)) << "\n";
                m << "GObjects:   " << Printer::Hex(gobjects, sizeof(uintptr_t))
                  << " (off " << Printer::Hex(gobjects - entry, sizeof(uintptr_t)) << ")\n";
                m << "GNames:     " << Printer::Hex(gnames, sizeof(uintptr_t))
                  << " (off " << Printer::Hex(gnames - entry, sizeof(uintptr_t)) << ")";
                Utils::Messagebox(m.str(), (MB_OK | MB_ICONINFORMATION));
            }
